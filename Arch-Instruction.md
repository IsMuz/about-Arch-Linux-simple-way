...
```bash
# Это на будущее, если потребуется поставить Windows
# Linux прекрасно обходится без efi раздела, но все же /boot лучше вынести на отдельный раздел + с btrfs grub не грузится
